...
Other workflow software emerging from the material chemistry community includes `atomate2` [@atomate2_zenodo; @atomate2_paper], `AiiDA` [@aiida_arxiv; @aiida_website; @aaida_paper1; @aaida_paper2] and `QuAcc` [@quacc_github].
...
Non-essential dependencies can be installed based on the desired calculator and interfaces to be used, all via conda.

The software implements various calculators (xTB [@xtb], tblite [@tblite; @tblite_paper], AIMNET2 [@aimnet2], UMA [@uma]) via ASE[@ase; @ase_paper1; @ase_paper2], which can be used interchangeably in various ensemble routines. Other core libraries leveraged throughout the codebase are `numpy` [@numpy] for algebraic manipulations, `networkx` [@networkx; @networkx_proc; @networkx_tech] for graph utilities and `prism_pruner` [@prism_pruner] for the removal of duplicates. `Sella` [@sella; @sella_paper1; @sella_paper2; @sella_paper3] provides a saddle point optimizer, and `rdkit` [@rdkit; @rdkit_zenodo] is used for `ETKDG` [@etkdg] conformational searches and SMARTS substructure matching. The `ML-FSM` library [@mlfsm; @mlfsm_paper] provides a two-ended TS search utility complementing ASE's implementation of the `NEB` method [@neb].
