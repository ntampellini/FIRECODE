...
The software implements various calculators (xTB [@xtb], tblite [@tblite; @tblite_paper], AIMNET2 [@aimnet2], UMA [@uma]) via ASE[@ase], which can be used interchangeably in various ensemble routines. Other core libraries leveraged throughout the codebase are `numpy` [@numpy] for algebraic manipulations, `networkx` [@networkx] for graph utilities and `prism_pruner` [@prism_pruner] for the removal of duplicates. `Sella` [@sella; @sella_paper1; @sella_paper2; @sella_paper3] provides a saddle point optimizer, and `rdkit` [@rdkit] is used for `ETKDG` [@etkdg] conformational searches and SMARTS substructure matching. The `ML-FSM` library [@mlfsm; @mlfsm_paper] provides a two-ended TS search utility complementing ASE's implementation of the `NEB` method [@neb].
