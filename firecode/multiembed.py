...
def multiembed_bifunctional(embedder):
    ...
    arrangements = [
        ((ix_1, ix_2), (iy_1, iy_2))
        for ((ix_1, ix_2), (iy_1, iy_2)) in permutations(pairs, 2)
        if ix_1 != iy_1 and ix_2 != iy_2
    ]
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i, process in enumerate(as_completed(processes)):
            ...
            embedder.log(
                f"--> Child process {i + 1:3}/{len(arrangements):3}: generated {len(structures):4} candidates in {time_to_string(elapsed, verbose=True)}."
            )
