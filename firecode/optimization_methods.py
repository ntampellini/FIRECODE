...
from firecode.ase_manipulations import ase_neb, ase_popt, ase_popt_with_alpb
...
def hyperNEB(embedder, atoms, coords, ids, constrained_indices, title="temp"):
    ...
    """
    reagents = get_reagent(
        embedder, atoms, coords, ids, constrained_indices, method=embedder.options.theory_level
    )
    ...
    ts_coords, ts_energy, energies, success = ase_neb(
        embedder,
        atoms,
        reagents,
        products,
...
def get_product(embedder, atoms, coords, ids, constrained_indices, method="PM7"):
    ...
    if len(ids) == 2:
        ...
        threshold_dists = [
            bond_factor * (pt.covalent_radius(atoms[a]) + pt.covalent_radius(atoms[b]))
        ...
        while not np.all(
            [reactive_dists[i] < threshold_dists[i] for i, _ in enumerate(constrained_indices)]
        ):
        ...
        if np.all(
            [new_reactive_dists[i] < threshold_dists[i] for i, _ in enumerate(constrained_indices)]
        ):
    ...
    threshold_dist = bond_factor * (
        pt.covalent_radius(atoms[constrained_indices[0, 0]])
    ...
    new_reactive_dist = norm_of(
        newcoords[constrained_indices[0, 0]] - newcoords[constrained_indices[0, 0]]
    )
...
def get_reagent(embedder, atoms, coords, ids, constrained_indices, method="PM7"):
    ...
    if len(ids) == 2:
        ...
        coords, _, _ = opt_func(
            atoms, coords, constrained_indices=constrained_indices, method=method
        )
    ...
    coords, _, _ = opt_func(
        atoms, coords, constrained_indices=np.array([constrained_indices[0]]), method=method
    )
