...
def automep(embedder, n_images=9):
    ...
    # graph = graphize(mol.atoms, mol.coords)
    ...
    mep = interpolate_structures(mol.atoms,
                                 align_structures(np.array([coords,        
                                                            ts_guess,
                                                            embedder.objects[1].coords[0]])),
                                 n=n_images,
                                 method='linear')
    ...
    for g, geom in enumerate(mep):
        if g not in (0, n_images-1):
            ...
            mep[g] = Atoms(mol.atoms, positions=opt_geom)
...
def get_exocyclic_dihedrals(graph, cycle):
    ...
    exo_dihs = []
    for index in cycle:
        for exo_id in graph.neighbors(index):
            if exo_id not in cycle:
                dummy1 = next(i for i in cycle if i not in (exo_id, index) and i in graph.neighbors(index))
                dummy2 = next(i for i in cycle if i not in (exo_id, index, dummy1) and i in graph.neighbors(dummy1))
